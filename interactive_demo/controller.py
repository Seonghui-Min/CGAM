...
class InteractiveController:
    ...
    def add_click(self, x, y, is_positive, radius):
        self.states.append({
            'clicker': self.clicker.get_state(),
            'predictor': self.predictor.get_states()
        })

        click = clicker.Click(is_positive=is_positive, coords=(y, x), radius=radius)
        self.clicker.add_click(click)
        pred, _, _, attention, _, _ = self.predictor.get_prediction(self.clicker)
        if attention is not None:
            ...
            image = Image.fromarray((attention*255).astype(np.uint8))
            image.save(f'./att_vis/{len(self.probs_history)}.png')
