...
class InteractiveDemoApp(ttk.Frame):
    def __init__(self, master, args, model):
        ...
        self.master = master
        master.title("Interactive Segmentation Tool")
        master.withdraw()
        ...
        self.brs_modes = ['NoBRS', 'RGB-BRS', 'DistMap-BRS', 'f-BRS-A', 'f-BRS-B', 'f-BRS-C', 'CGAM']
    ...
    def _init_state(self):
        self.state = {
            ...
            'brs_mode': tk.StringVar(value='CGAM'),
    ...
    def _load_image_callback(self):
        ...
        if self._check_entry(self):
            self.filenames = filedialog.askopenfilenames(parent=self.master, filetypes=[
                ("Images", "*.jpg *.JPG *.jpeg *.png *.bmp *.tiff *.tif"),
                ("All files", "*.*"),
    ...
    def _click_callback(self, is_positive, x, y):
        ...
        if self._check_entry(self):
            self.controller.add_click(x, y, is_positive, self.circle_radius)
    ...
    def _vis_click_radius(self):
        ...
        self.circle = self.canvas.create_oval(
            ...
            fill=""  # Inner color (transparent)
        )
        
        self.canvas_frame.bind("<Enter>", self.show_circle)
        self.canvas_frame.bind("<Leave>", self.hide_circle)
        self.canvas.bind("<Motion>", self.update_circle_position)
        self.canvas.bind("<MouseWheel>", self.update_circle_radius)
        
        self.hide_circle(None)
        
    def show_circle(self, event):
        self.canvas.itemconfig(self.circle, state="normal")

    def hide_circle(self, event):
        self.canvas.itemconfig(self.circle, state="hidden")

    def update_circle_position(self, event):
        # Move the circle according to the mouse position
        self.circle_center = (event.x, event.y)
        self.update_circle()

    def update_circle_radius(self, event):
        # Adjust the circle's radius based on the mouse wheel event
        delta = event.delta
        if delta > 0:
            self.circle_radius += 5
        else:
