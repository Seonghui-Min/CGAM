...
def draw_points(image, points, color, radius):
    ...
    for p, r in zip(points, radius):
        image = cv2.circle(image, (int(p[1]), int(p[0])), r, color, 2)
...
def draw_with_blend_and_clicks(img, mask=None, alpha=0.6, clicks_list=None, pos_color=(0, 255, 0, 255),
                               neg_color=(255, 0, 0, 255), radius=4):
    result = img.copy()
    ...
    if clicks_list is not None and len(clicks_list) > 0:
        ...
        pos_radius = [click.radius for click in clicks_list if click.is_positive]
        neg_radius = [click.radius for click in clicks_list if not click.is_positive]

        result = draw_points(result, pos_points, pos_color, pos_radius)
        result = draw_points(result, neg_points, neg_color, neg_radius)
