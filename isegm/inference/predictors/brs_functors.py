...
from .brs_losses import BRSMaskLoss, GBRSLoss
...
class CGAMOptimizer(BaseOptimizer):
    def __init__(self, *args, scale_act=None, reg_weight=0.1, **kwargs):
        super().__init__(*args, **kwargs)
        self.scale_act = scale_act
        self.reg_weight = 1.0
        ...
        self.gbrs_loss = GBRSLoss()
    
    def init_optimizer(self, predictor, device):
        self.device = device
        
        learning_rate = 0.1 * (0.5**1)
        self.optimizer = torch.optim.Adam(predictor.cgam.parameters(), lr=learning_rate)
        
    def optimize(self, pred_logits, pos_mask, neg_mask, newest_click_mask, i=None):
        self.optimizer.zero_grad()
        
        loss, early_stopping = self.gbrs_loss(pred_logits, pos_mask, neg_mask)
        
        diff_mask = (pred_logits - self.prev_logits) * (1 - newest_click_mask)
        reg_loss = self.reg_weight * torch.mean(diff_mask**2)
        loss = loss + reg_loss
