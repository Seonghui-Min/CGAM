...
from .brs import InputBRSPredictor, FeatureBRSPredictor, HRNetFeatureBRSPredictor, CGAMPredictor
from .brs_functors import InputOptimizer, ScaleBiasOptimizer, CGAMOptimizer
...
def get_predictor(net, brs_mode, device,
    ...
    ##########################################################
        ...
        insertion_mode = {'CGAM': 'after_aspp'}[brs_mode]

        opt_functor = CGAMOptimizer(prob_thresh=prob_thresh,
                                         with_flip=with_flip,
                                         optimizer_params=lbfgs_params_,
                                         **brs_opt_func_params)
        ...
        if isinstance(net, DistMapsHRNetModel):
            FeaturePredictor = HRNetFeatureBRSPredictor
            insertion_mode = {'after_aspp': 'A'}[insertion_mode]
        else:
            FeaturePredictor = CGAMPredictor
